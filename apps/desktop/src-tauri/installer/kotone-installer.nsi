...
; Optional Tauri lifecycle hooks.
{{#if installer_hooks}}
!include "{{installer_hooks}}"
{{/if}}

; Kotone's zero-dependency MUI2 theme. All resources are checked into the
; repository, so local and CI builds use the same inputs.
...
; 1. Welcome Page
!define MUI_WELCOMEPAGE_TITLE "$(kotoneWelcomeTitle)"
!define MUI_WELCOMEPAGE_TEXT "$(kotoneWelcomeText)"
...
!define MUI_FINISHPAGE_TITLE "$(kotoneFinishTitle)"
!define MUI_FINISHPAGE_TEXT "$(kotoneFinishText)"
...
LangString kotoneWelcomeTitle ${LANG_SIMPCHINESE} "建立连接"
